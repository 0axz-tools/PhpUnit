# modifikasi agar run di python 3
import re, sys, requests, os, random, string, time, codecs, base64, subprocess
from time import time as timer
import requests, re, os, sys, codecs, json, argparse
from multiprocessing.dummy import Pool
import requests, os, sys, time, codecs, base64, subprocess
from time import time as timer
import time
...
# Hapus baris berikut karena tidak diperlukan di Python 3
# reload(sys)
...
def urlfix(url):
    if url and url[-1] == '/':
        ...
        if site:
            url = site[0]
    if url and url[:7] != 'http://' and url[:8] != 'https://':
        url = 'http://' + url
    return url
